...
PdfSplit.exe       １ページずつに分割（複数ファイル非対応）
 ...
 分割後のファイル名は、元のファイル名に`_ページ数`となる。

 ページ数は0から始まる。（PdfSplitU.exeと異なる）

PdfSplitU.exe      １ページずつに分割（複数ファイル非対応） ※保護ファイル対応

 分割後のファイル名は、元のファイル名に`_ページ数`となる。

 ページ数は1から始まる。（旧バージョンは0から）
...
PdfDevide2.exe      指定したページで分割（複数ファイル非対応）

　ダイアログに分割する先頭ページ数をカンマ（,）で区切って入力します。

　１０ページあるファイルに対して、`3,7`と入力すると、１～２ページのファイル、３～６ページのファイル、７～１０ページのファイルができます。
...
PdfPickup.exe　　指定したページを抽出（複数ファイル非対応）

　ダイアログに抽出するページ数を入力します。

　`1,3-5,8`と入力すると、１，３，４，５，８ページをコピーして別のファイルにします。

　ファイル名は、指定したファイルのファイル名に`_pickup`を付けたものになります。指定した元のファイルはそのままです。
